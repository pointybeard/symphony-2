...
class Session
{
    ...
    {
        if (!self::$initialized) {
            if (false == is_object(\Symphony::Database()) || false == \Symphony::Database()->isConnected()) {
                return false;
            }
            ...
            if ('' == session_id()) {
                ...
                ini_set('session.gc_divisor', (string) \Symphony::Configuration()->get('session_gc_divisor', 'symphony'));
            }
            ...
            if ('' == session_id()) {
                if (headers_sent()) {
                    throw new Exceptions\SymphonyException('Headers already sent. Cannot start session.');
                }
    ...
    {
        if (HTTP_HOST != null) {
            if (true == preg_match('/(localhost|127\.0\.0\.1)/', HTTP_HOST)) {
    ...
    public static function write($id, $data)
    {
        ...
        if (!$session_data) {
            $empty = true;
            if (true == function_exists('session_status') && PHP_SESSION_ACTIVE === session_status()) {
                $unserialized_data = self::unserialize($data);
