...
abstract class AbstractPage
{
     ...
     * @var int
     */
    public const HTTP_STATUS_MOVED_PERMANENT = 301;
     ...
     * @var int
     */
    public const HTTP_STATUS_ERROR = 500;
     ...
     */
    public static function getHttpStatusValue($status_code = null)
    {
        if (!$status_code) {
     ...
     */
    public static function getHeaderStatusString($status_code)
    {
     ...
     */
    public static function setHttpStatusValue($status_code, $string_value)
    {
        if (!$string_value) {
    ...
    public function addHeaderToPage($name, $value = null, $response_code = null)
    {
        $this->_headers[strtolower($name)] = array(
            'header' => $name.(null === $value ? null : ":{$value}"),
     ...
     * @param int $status_code
     */
    public static function renderStatusCode($status_code)
    {
    ...
    protected function __renderHeaders()
    {
        ...
        foreach ($this->_headers as $key => $value) {
            ...
            if ('status' == $key && isset($value['response_code'])) {
     ...
     *
     * @see https://github.com/symphonycms/symphony-2/issues/1187
    ...
    public function isRequestValid()
    {
        ...
        if ('POST' === server_safe('REQUEST_METHOD') && (int) server_safe('CONTENT_LENGTH') > \General::convertHumanFileSizeToBytes($max_size)) {
            return false;
        }
