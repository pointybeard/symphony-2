...
class HtmlPage extends AbstractPage
{
    ...
    {
        if (true === $duplicate || (false === $duplicate && false === $this->checkElementsInHead($path, 'src'))) {
    ...
    {
        if (true === $duplicate || (false === $duplicate && false === $this->checkElementsInHead($path, 'href'))) {
     ...
     * @return string
     */
    public function __buildQueryString(array $exclude = [], $filters = null)
    {
        $exclude[] = 'page';
        if (null === $filters) {
