 ...
 * @since Symphony 2.3
 */
class PageManager extends Symphony\AbstractManager
{
    /**
     ...
     *                           The new Page handle, generated using `self::createHandle`
    ...
    public static function createPageFiles($new_path, $new_handle, $old_path = null, $old_handle = null)
    {
        ...
        if (self::writePageFiles($new, $data)) {
            // Remove the old file, in the case of a rename
            Symphony\General::deleteFile($old);
    ...
    /**
     * A wrapper for `Symphony\General::writeFile`, this function takes a `$path`
    ...
    {
        return Symphony\General::writeFile($path, $data, \Symphony::Configuration()->get('write_mode', 'file'));
    }
    ...
    public static function editPageChildren($page_id = null, $page_path = null)
    {
        ...
        $children = self::fetchChildPages($page_id);
        ...
        foreach ($children as $child) {
            ...
            if (!self::createPageFiles($page_path, $child['handle'], $child['path'], $child['handle'])) {
                $success = false;
            }

            if (!self::edit($child_id, $fields)) {
                $success = false;
            }

            $success = self::editPageChildren($child_id, $page_path.'/'.$child['handle']);
        }
    ...
    public static function delete($page_id = null, $delete_files = true)
    {
        ...
        if ($delete_files) {
            $page = self::fetchPageByID($page_id, array('path', 'handle'));

            if (empty($page)) {
                return false;
            }

            $can_proceed = self::deletePageFiles($page['path'], $page['handle']);
        }
     ...
     *                          eg. article/read. If a page has no parents, this parameter should be null.
     * @param string $handle
     *                          A Page handle, generated using `self::createHandle`
     *
     * @throws Exception
     *
     * @return bool
    ...
    {
        $file = self::resolvePageFileLocation($page_path, $handle);
        ...
        // Delete it:
        if (Symphony\General::deleteFile($file)) {
            return true;
        }
     ...
     * @param bool   $hierarchical  (optional)
     ...
     */
    public static function fetch($include_types = true, array $select = [], array $where = [], $order_by = null, $hierarchical = false)
    {
        if ($hierarchical) {
        ...
        if ($include_types) {
            foreach ($pages as &$page) {
                $page['type'] = self::fetchPageTypes($page['id']);
            }
        }
     ...
     *                    null is returned.
     */
    public static function fetchPageByID($page_id = null, array $select = [])
    {
        if (null === $page_id) {
            return null;
        }
        ...
        $page = self::fetch(true, $select, array(
            sprintf('id IN (%s)', implode(',', $page_id)),
        ));

        return 1 == count($page) ? array_pop($page) : $page;
    }

    /**
     * Returns Pages that match the given `$type`. If no `$type` is provided
     * the function returns the result of `self::fetch`.
     *
     * @param string $type
    ...
    public static function fetchPageByType($type = null)
    {
        ...
        return 1 == count($pages) ? array_pop($pages) : $pages;
    }
     ...
     *                    null is returned.
     */
    public static function fetchChildPages($page_id = null, array $select = [])
    {
        if (null === $page_id) {
            return null;
        }
        ...
        return self::fetch(false, $select, array(
    ...
    public static function fetchPageTypes($page_id = null)
    {
        return \Symphony::Database()->fetchCol('type', sprintf(
            ...
            ORDER BY
                pt.type ASC',
            (null === $page_id ? '1' : sprintf('pt.page_id = %d', $page_id))
        ));
    }
    ...
    public static function fetchAvailablePageTypes()
    {
        ...
        $types = self::fetchPageTypes();

        return !empty($types) ? Symphony\General::array_remove_duplicates(array_merge($system_types, $types)) : $system_types;
    }
    ...
    public static function getChildPagesCount($page_id = null)
    {
        ...
        $children = self::fetch(false, array('id'), array(
            sprintf('parent = %d', $page_id),
        ));
    ...
    {
        return (bool) \Symphony::Database()->fetchRow(0, sprintf(
            "SELECT
                pt.id
            FROM
                `tbl_pages_types` AS pt
    ...
    {
        return (bool) \Symphony::Database()->fetchVar('id', 0, sprintf(
            'SELECT
                p.id
            FROM
                `tbl_pages` AS p
    ...
    {
        return PAGES.'/'.self::createFilePath($path, $handle).'.xsl';
    }
    ...
    {
        $path = self::resolvePage($page_id, 'title');
    ...
    {
        $path = self::resolvePage($page_id, 'handle');
    ...
    {
        return (bool) \Symphony::Database()->fetchVar('count', 0, "SELECT COUNT(*) AS `count` FROM `tbl_pages` WHERE `data_sources` REGEXP '[[:<:]]{$handle}[[:>:]]' ") > 0;
    }
    ...
    {
        return (bool) \Symphony::Database()->fetchVar('count', 0, "SELECT COUNT(*) AS `count` FROM `tbl_pages` WHERE `events` REGEXP '[[:<:]]{$handle}[[:>:]]' ") > 0;
    }
}
