 ...
 * @since Symphony 2.3
 */
class ResourceManager extends Symphony\AbstractManager
{
    /**
     ...
     * @var int
     */
    public const RESOURCE_TYPE_EVENT = 20;
     ...
     * @var int
     */
    public const RESOURCE_TYPE_DS = 21;
    ...
    {
        switch ($type) {
            case self::RESOURCE_TYPE_EVENT:
                return 'events';
            case self::RESOURCE_TYPE_DS:
                return 'data_sources';
        }
    }
    ...
    public static function getSortingField($type)
    {
        ...
        return null === $result ? 'name' : $result;
    }
     ...
     */
    public static function fetch($type, array $select = [], array $where = [], $order_by = null)
    {
        $manager = self::getManagerFromType($type);

        if (!isset($manager)) {
            throw new Exceptions\SymphonyException(__('Unable to find a Manager class for this resource.'));
        }
        ...
        foreach ($resources as &$r) {
            ...
            if (isset($r['source']) && Symphony\General::intval($r['source']) > 0) {
                $section = SectionManager::fetch($r['source']);

                if (false !== $section) {
                    $r['source'] = array(
                        'name' => Symphony\General::sanitize($section->get('name')),
                        'handle' => Symphony\General::sanitize($section->get('handle')),
                        'id' => $r['source'],
                    );
                } else {
                    unset($r['source']);
                ...
                $r['source'] = array(
                    'name' => ucwords(Symphony\General::sanitize($r['source'])),
                    'handle' => Symphony\General::sanitize($r['source']),
                );
            ...
            // Resource provided by extension?
                ...
                if (!empty($extension)) {
                    ...
                    $r['source'] = array(
                        'name' => Symphony\General::sanitize($extension['name']),
                        'handle' => Symphony\Lang::createHandle($extension['name']),
                    );
                }
            }
        }
        ...
        if (null !== $order_by && !empty($resources)) {
        ...
        if (is_array($pages) && 1 == count($pages)) {
            ...
            if (!in_array($r_handle, explode(',', $result))) {
                ...
                return PageManager::edit($page_id, array(
                    $col => Symphony\DatabaseWrappers\MySql::cleanValue($result),
                ));
            }
        }
     ...
     * @return bool
     */
    public static function setPages($type, $r_handle, $pages = [])
    {
        if (!is_array($pages)) {
            $pages = [];
        }
        ...
        // Get attached pages
        $attached_pages = self::getAttachedPages($type, $r_handle);
        ...
        foreach ($diff_to_attach as $diff_page) {
            self::attach($type, $r_handle, $diff_page);
        }
        ...
        foreach ($diff_to_detach as $diff_page) {
            self::detach($type, $r_handle, $diff_page);
        }
