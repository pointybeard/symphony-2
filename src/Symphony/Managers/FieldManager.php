 ...
 */
class FieldManager extends Symphony\AbstractManager implements Interfaces\FileResourceInterface
{
    /**
     ...
     * @var array
     */
    private static $initialiasedFields = [];
                ...
                // We don't have an instance of this field, so let's set one up
                    ...
                    if (is_array($context) && !empty($context)) {
                        try {
                            ...
                            throw new Exceptions\SymphonyException(__(
                    ...
                    self::$initialiasedFields[$f['id']] = $field;
                }
    ...
    public static function fetchFieldIDFromElementName($element_name, $section_id = null)
    {
        if (null === $element_name) {
            ...
            $schema_sql = empty($element_names) ? null : sprintf(
                ...
                ORDER BY `sortorder` ASC",
                (null !== $section_id ? sprintf('AND `parent_section` = %d', $section_id) : ''),
    ...
    public static function isTextFormatterUsed($text_formatter_handle)
    {
        ...
        if (!empty($fields)) {
            foreach ($fields as $field) {
                ...
                } catch (Exceptions\DatabaseException $ex) {
