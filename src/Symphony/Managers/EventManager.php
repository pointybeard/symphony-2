 ...
 */
class EventManager extends Symphony\AbstractManager implements Interfaces\FileResourceInterface
{
    /**
    ...
    {
        $result = [];
        $structure = Symphony\General::listStructure(EVENTS, '/event.[\\w-]+.php/', false, 'ASC', EVENTS);
        ...
        if (is_array($extensions) && !empty($extensions)) {
            foreach ($extensions as $e) {
                ...
                $tmp = Symphony\General::listStructure(EXTENSIONS."/$e/events", '/event.[\\w-]+.php/', false, 'ASC', EXTENSIONS."/$e/events");
