...
class EmailGatewayManager extends Symphony\AbstractManager implements Interfaces\FileResourceInterface
{
    ...
    public static function setDefaultGateway($name)
    {
        if (self::__getClassPath($name)) {
            ...
            throw new Exceptions\EmailGatewayException(__('This gateway can not be found. Can not save as default.'));
        }
    }
    ...
    {
        return preg_replace(['/^email\./i', '/\.php$/i'], '', $filename);
    }
    ...
    {
        $result = [];
        $structure = Symphony\General::listStructure(EMAILGATEWAYS, '/[^\.]+\.php/', false, 'ASC', EMAILGATEWAYS);
        ...
        if (is_array($extensions) && !empty($extensions)) {
            foreach ($extensions as $e) {
                ...
                $tmp = Symphony\General::listStructure(EXTENSIONS."/{$e}/email-gateways", '/(email\.)?[\\w-]+\.php/', false, 'ASC', EXTENSIONS."/{$e}/email-gateways");
