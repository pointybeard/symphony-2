 ...
 */
class ExtensionManager extends Symphony\AbstractManager implements Interfaces\FileResourceInterface
{
    /**
     ...
     * @var array
     */
    private static $enabledExtensions = [];
    ...
    /**
     * The constructor will populate the `$subscriptions` variable from
     * the `tbl_extension` and `tbl_extensions_delegates` tables.
     */
    public function __construct()
    {
        if (empty(self::$subscriptions) && \Symphony::Database()->isConnected()) {
            ...
            foreach ($subscriptions as $subscription) {
                self::$subscriptions[$subscription['delegate']][] = $subscription;
            }
        }
    }
    ...
    /**
     * Populates the `ExtensionManager::$extensions` array with all the
     * extensions stored in `tbl_extensions`. If `ExtensionManager::$extensions`
     * isn't empty, passing true as a parameter will force the array to update.
     *
     * @param bool $update
     *                     Updates the `ExtensionManager::$extensions` array even if it was
    ...
    {
        if (empty(self::$extensions) || $update) {
            self::$extensions = \Symphony::Database()->fetch('SELECT * FROM `tbl_extensions`', 'name');
        }
    }
    ...
    public static function fetchStatus($about)
    {
        ...
        if (isset($about['handle']) && array_key_exists($about['handle'], self::$extensions)) {
            if ('enabled' == self::$extensions[$about['handle']]['status']) {
                $return[] = Symphony\AbstractExtension::EXTENSION_ENABLED;
            } else {
                $return[] = Symphony\AbstractExtension::EXTENSION_DISABLED;
            }
        } else {
            $return[] = Symphony\AbstractExtension::EXTENSION_NOT_INSTALLED;
        }

        if (isset($about['handle'], $about['version']) && self::__requiresUpdate($about['handle'], $about['version'])) {
            $return[] = Symphony\AbstractExtension::EXTENSION_REQUIRES_UPDATE;
        }
    ...
    public static function fetchInstalledVersion($name)
    {
        ...
        return isset(self::$extensions[$name]) ? self::$extensions[$name]['version'] : null;
    }
    ...
    public static function fetchExtensionID($name)
    {
        ...
        return self::$extensions[$name]['id'];
    }
    ...
    public static function getProvidersOf($type = null)
    {
        ...
        if (empty(self::$providers)) {
            ...
            foreach (self::listInstalledHandles() as $handle) {
                ...
                self::$providers = array_merge_recursive(self::$providers, $obj->providerOf());
            }
        }
    ...
    public static function getCacheProvider($key = null, $reuse = true)
    {
        ...
        if (false === $reuse) {
            ...
            self::$_pool[$cacheDriver] = new Symphony\Cacheable($cacheable);
        }
    ...
    public static function enable($name)
    {
        ...
        if (self::__requiresInstallation($name) && false === $obj->install()) {
        ...
        } elseif (($about = self::about($name)) && false !== ($previousVersion = self::__requiresUpdate($name, $about['version']))) {
    ...
    public static function uninstall($name)
    {
        ...
        try {
            ...
            self::__canUninstallOrDisable($obj);
            $obj->uninstall();
        } catch (Exceptions\SymphonyErrorPageException $ex) {
            // Create a consistant key
            $key = str_replace('-', '_', $ex->getTemplateName());

            if ('missing_extension' !== $key) {
                throw $ex;
            }
        }
     ...
     * @throws Exception
     */
    private static function __canUninstallOrDisable(Symphony\AbstractExtension $obj)
    {
        ...
        if (is_dir(EXTENSIONS."/{$extension_handle}/fields")) {
            foreach (glob(EXTENSIONS."/{$extension_handle}/fields/field.*.php") as $file) {
                ...
                if (FieldManager::isFieldUsed($type)) {
                    throw new Exceptions\SymphonyException(
    ...
    public static function notifyMembers($delegate, $page, array $context = array())
    {
        ...
        if (isset(self::$subscriptions[$delegate])) {
            foreach (self::$subscriptions[$delegate] as $subscription) {
        ...
        $profiling = \Symphony::Profiler() instanceof Symphony\Profiler;
    ...
    {
        if (empty(self::$enabledExtensions) && \Symphony::Database()->isConnected()) {
            self::$enabledExtensions = \Symphony::Database()->fetchCol(
                'name',
                "SELECT `name` FROM `tbl_extensions` WHERE `status` = 'enabled'"
            );
        }

        return self::$enabledExtensions;
    }
    ...
    {
        $result = [];
        $extensions = Symphony\General::listDirStructure(EXTENSIONS, $filter, false, EXTENSIONS);
    ...
    public static function fetch(array $select = array(), array $where = array(), $order_by = null)
    {
        ...
        if (empty($select) && empty($where) && null === $order_by) {
            return $extensions;
        }
        ...
        if (null !== $order_by) {
            ...
            $order = ('desc' == $order_by[1]) ? SORT_DESC : SORT_ASC;
            $sort = $order_by[0];
        ...
        if (file_exists(self::__getClassPath($name).'/extension.meta.xml')) {
            try {
                ...
                if (null === $rootNamespace) {
                    throw new Exceptions\SymphonyException(__('Missing default namespace definition.'));
                } else {
            ...
            if ($release = $xpath->query("//ext:release[@version='$latest_release_version']", $extension)->item(0)) {
                ...
                if (!empty($required_min_version) && version_compare($current_symphony_version, $required_min_version, '<')) {
                    $about['status'][] = Symphony\AbstractExtension::EXTENSION_NOT_COMPATIBLE;
        ...
        if (!isset(self::$_pool[$name])) {
            ...
            if (!is_file($path)) {
                ...
                try {
                    ...
                    throw new Exceptions\SymphonyException($errMsg, 0, $ex);
                }
            }
