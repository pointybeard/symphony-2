...
class XmlElement implements \IteratorAggregate
{
     ...
     *                      if it is in the `self::no_end_tags`
     */
    ...
    {
        if ($this === $child) {
            throw new Exceptions\SymphonyException(__('Can not add the element itself as one of its child'));
        }
    }
     ...
     * @return bool
     */
    public function insertChildAt($index, self $child = null)
    {
        if (!is_numeric($index)) {
            return false;
        }
     ...
     * @return bool
     */
    public function replaceChildAt($index, self $child = null)
    {
        if (!is_numeric($index)) {
            return false;
        }
    ...
    public function generate($indent = false, $tab_depth = 0, $has_parent = false)
    {
        ...
        if (!empty($attributes)) {
            foreach ($attributes as $attribute => $value) {
                $length = strlen($value);
                if (0 !== $length || 0 === $length && $this->_allowEmptyAttributes) {
        ...
        if ($this->getNumberOfChildren() > 0 || 0 !== strlen($value) || !$this->_selfclosing) {
            $result .= '>';
        ...
        // Empty elements:
            ...
            if ('xml' === $this->_elementStyle) {
                $result .= ' />';
            } elseif (in_array($this->_name, self::$no_end_tags) || ('!--' === substr($this->getName(), 0, 3))) {
                $result .= '>';
            } else {
    ...
    /**
     * This helper function is used by `self::convertFromDOMDocument`
     ...
     * @return XMLElement
     */
    protected static function convert(self $root = null, \DOMNode $node)
    {
        $el = new self($node->tagName);
        ...
        if ($node->hasChildNodes()) {
            foreach ($node->childNodes as $childNode) {
                if ($childNode instanceof \DOMCdataSection) {
                    ...
                    if (false === $childNode->isWhitespaceInElementContent()) {
