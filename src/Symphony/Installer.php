...
class Installer extends Administration
{
    ...
    protected function __construct()
    {
        ...
        $settings = json_decode(file_get_contents(INCLUDES . '/Install/config_default.json'), true);
        ...
        Handlers\GenericExceptionHandler::initialise(Symphony::Log());
        Handlers\GenericErrorHandler::initialise(Symphony::Log());
    ...
    public function run(): void
    {
        ...
        if (is_null(Symphony::Log()) || !file_exists(Symphony::Log()->getLogPath())) {
            self::render(new Installer\Page('missing-log'));
        }
        ...
        if (!empty($errors)) {
            ...
            self::render(new Installer\Page('requirements', array(
                'errors'=> $errors
            )));
        }

        // Check for unattended installation
        $unattended = self::checkUnattended();
        if (!empty($unattended)) {
            // Merge unattended information with the POST
            self::$postData = array_replace_recursive($unattended, self::$postData);
        }

        // If language is not set and there is language packs available, show language selection pages
        if (!isset(self::$postData['lang']) && count(Lang::getAvailableLanguages(false)) > 1) {
            self::render(new Installer\Page('languages'));
        }

        // Check for configuration errors and, if there are no errors, install Symphony!
        if (isset(self::$postData['fields'])) {
            ...
            } elseif (isset(self::$postData['action']['install'])) {
                $disabledExtensions = self::install();

                self::render(new Installer\Page('success', array(
        ...
        self::render(new Installer\Page('configuration', array(
            'errors' => $errors,
    ...
    private static function checkRequirements(): array
    {
        ...
        if (!file_exists(INCLUDES . '/Install/install.sql') || !is_readable(INCLUDES . '/Install/install.sql')) {
            $errors[] = array(
     ...
     */
    private static function checkUnattended(): bool
    {
     ...
     */
    protected static function abort($message, $start): void
    {
        ...
        self::render(new Installer\Page('failure'));
    }
    ...
    private static function install(): array
    {
        ...
        if (!is_array($conf)) {
            self::abort('The configuration is not an array, can not continue', $start);
        }
        ...
        $rewriteBase = ltrim(preg_replace('/\/install$/i', null, dirname($_SERVER['PHP_SELF'])), '/');
        $htaccess = str_replace(
            '<!-- REWRITE_BASE -->', $rewriteBase,
            file_get_contents(INCLUDES . '/Install/htaccess.txt')
        );
        ...
        foreach (new DirectoryIterator(EXTENSIONS) as $e) {
            ...
            try {
                if (!Managers\ExtensionManager::enable($handle)) {
                    $disabledExtensions[] = $handle;
        ...
        if (isset($_REQUEST['lang']) && $_REQUEST['lang'] != 'en') {
            ...
            if (in_array('lang_' . $language['handle'], Managers\ExtensionManager::listInstalledHandles())) {
    ...
    protected static function render(Installer\Page $page): void
    {
