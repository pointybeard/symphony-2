...
/**
 * The `AuthorDatasource` extends the base `AbstractDatasource` class and allows
 * the retrieval of Author information from the current Symphony installation.
 *
 * @since Symphony 2.3
 */
abstract class Author extends Symphony\AbstractDatasource
{
    ...
    public function execute(array &$param_pool = null)
    {
        ...
        if ((!is_array($authors) || empty($authors)) && 'yes' === $this->dsParamREDIRECTONEMPTY) {
            ...
            foreach ($authors as $author) {
                if (isset($this->dsParamPARAMOUTPUT)) {
                    foreach ($this->dsParamPARAMOUTPUT as $param) {
                        ...
                        $param_pool[$param_key][] = ('name' === $param ? $author->getFullName() : $author->get($param));
                        ...
                        if ($singleParam) {
                            ...
                            $param_pool[$key][] = ('name' === $param ? $author->getFullName() : $author->get($param));
                        }
                    }
                }
                ...
                if (!isset($this->dsParamINCLUDEDELEMENTS) || !is_array($this->dsParamINCLUDEDELEMENTS) || empty($this->dsParamINCLUDEDELEMENTS)) {
                    ...
                    // Default Area
                    if (in_array('default-area', $this->dsParamINCLUDEDELEMENTS) && null !== $author->get('default_area')) {
                        // Section
