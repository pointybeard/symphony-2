...
class General
{
    ...
    {
        if (!is_array($rule) && ('' == $rule || null == $rule)) {
            return true;
        }

        if (!is_array($string) && ('' == $string || null == $rule)) {
            return true;
        }
    ...
    public static function createHandle($string, $max_length = 255, $delim = '-', $uriencode = false, $additional_rule_set = null)
    {
        ...
        // Trim it
        if ($max_length > 0) {
            $string = self::limitWords($string, $max_length);
        }
     ...
     * @param string|int $mode   (optional)
    ...
    public static function realiseDirectory($path, $mode = 0755, $silent = true)
    {
        ...
        try {
            ...
            if (false === $silent) {
                throw new Exceptions\SymphonyException(__('Unable to create path - %s', array($path)));
            }
    ...
    public static function deleteDirectory($dir, $silent = true)
    {
        try {
            ...
            if (false === $silent) {
                throw new Exceptions\SymphonyException(__('Unable to remove - %s', array($dir)));
            }
    ...
    {
        return true === $ignore_case ? self::array_iunique($array) : array_unique($array);
    }
    ...
    public static function array_to_xml(\XMLElement $parent, array $data, $validate = false)
    {
        foreach ($data as $element_name => $value) {
            ...
            if (is_array($value) || is_object($value)) {
                ...
                if (0 == $child->getNumberOfChildren()) {
                    continue;
                }
            } elseif (true === $validate && !self::validateXML(self::sanitize($value), $errors, false)) {
                continue;
            } else {
     ...
     * This method calls `self::checkFileWritable()` which properly checks
     * for permissions.
     *
     * @uses self::checkFileWritable()
     *
     * @param string     $file
     *                         the path of the file to write
     * @param mixed      $data
     *                         the data to write to the file
     * @param int|string $perm (optional)
    ...
    {
        if (false === static::checkFileWritable($file)) {
            return false;
        }
        ...
        if (false === fwrite($handle, $data, strlen($data))) {
            return false;
        }
    ...
    {
        try {
            if (false === static::checkFileDeletable($file)) {
                throw new Exceptions\SymphonyException(__('Denied by permission'));
            }
            if (!@file_exists($file)) {
                return true;
            }
            ...
            if (false === $silent) {
                throw new Exceptions\SymphonyException(__('Unable to remove file - %s', array($file)), 0, $ex);
            }
    ...
    public function getMimeType($file)
    {
        if (!empty($file)) {
            ...
            if (PHP_VERSION_ID >= 50300 && function_exists('finfo_open')) {
                ...
                if (null !== $mimetypes[strtolower($extension)]) {
    ...
    public static function listStructure($dir = '.', $filters = [], $recurse = true, $sort = 'asc', $strip_root = null, $exclude = [], $ignore_hidden = true)
    {
        ...
        foreach (scandir($dir) as $file) {
            ...
            } elseif ('regex' === $filter_type) {
                if (preg_match($filters, $file)) {
                    $files['filelist'][] = "$prefix$file";
                }
            } elseif ('file' === $filter_type) {
                if (in_array(self::getExtension($file), $filters)) {
                    $files['filelist'][] = "$prefix$file";
                }
            } elseif (null === $filter_type) {
                $files['filelist'][] = "$prefix$file";
            }
        }

        if (is_array($files['filelist'])) {
            ('desc' == $sort) ? rsort($files['filelist']) : sort($files['filelist']);
        }
     ...
     * the output of `self::formatFilesize` as well as reading values
    ...
    public static function createXMLDateObject($timestamp, $element = 'date', $date_format = 'Y-m-d', $time_format = 'H:i', $namespace = null)
    {
        ...
        $xDate = new \XMLElement(
            (null !== $namespace ? $namespace.':' : '').$element,
    ...
    public static function hash($input, $algorithm = 'sha1')
    {
        switch ($algorithm) {
            case 'pbkdf2':
            default:
                return Crytography\Pbkdf2::hash($input, $algorithm);
        }
    }
    ...
    public static function ensureType(array $params)
    {
        foreach ($params as $name => $param) {
            if (isset($param['optional']) && (true === $param['optional'])) {
                if (null === $param['var']) {
                    continue;
                }
    ...
    {
        if (is_numeric($value) && 1 === preg_match('/^[0-9]+$/i', (string) $value)) {
            return intval($value);
        }
