 ...
 */
class Sendmail extends Symphony\AbstractEmailGateway
{
    /**
    ...
    public function send()
    {
        ...
        try {
            ...
            foreach ($this->_recipients as $name => $email) {
                ...
                $name = \General::intval($name) > -1 ? \General::intval($name) : Symphony\EmailHelper::qEncode($name);
                $recipients[$name] = $email;
            }

            // Combine keys and values into a recipient list (name <email>, name <email>).
            $recipient_list = Symphony\EmailHelper::arrayToList($recipients);

            // Encode the subject
            $subject = Symphony\EmailHelper::qEncode((string) $this->_subject);

            // Build the 'From' header field body
            $from = empty($this->_sender_name)
                    ? $this->_sender_email_address
                    : Symphony\EmailHelper::qEncode($this->_sender_name).' <'.$this->_sender_email_address.'>';
            ...
            if (!empty($this->_reply_to_email_address)) {
                $reply_to = empty($this->_reply_to_name)
                            ...
                            : Symphony\EmailHelper::qEncode($this->_reply_to_name).' <'.$this->_reply_to_email_address.'>';
            }
            ...
            throw new Exceptions\EmailGatewayException($e->getMessage());
        }
