...
namespace Symphony\Symphony\EmailGateways;
 ...
 */
class Smtp extends Symphony\AbstractEmailGateway
{
    protected $_SMTP;
    ...
    public function send()
    {
        ...
        try {
            if (!is_a($this->_SMTP, 'SMTP')) {
                $this->_SMTP = new Symphony\Smtp($this->_host, $this->_port, $settings);
            }
            ...
            $this->_SMTP->sendMail(null === $this->_envelope_from ? $this->_sender_email_address : $this->_envelope_from, $this->_recipients, $this->_body);
    ...
    {
        if (null === $port) {
            $port = ('ssl' == $this->_protocol) ? 465 : 25;
        }
    ...
    {
        if (preg_match('%[\r\n]%', $envelope_from)) {
            throw new Exceptions\EmailValidationException(__('The Envelope From Address can not contain carriage return or newlines.'));
        }
    ...
    public function setConfiguration($config)
    {
        ...
        $this->setAuth(1 === (int) $config['auth']);
    ...
    public function getPreferencesPane()
    {
        ...
        $options = array(
            array('no', 'no' == $this->_secure, __('No encryption')),
            array('ssl', 'ssl' == $this->_secure, __('SSL encryption')),
            array('tls', 'tls' == $this->_secure, __('TLS encryption')),
        );
