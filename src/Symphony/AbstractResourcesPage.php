 ...
 */
abstract class AbstractResourcesPage extends Administration\AbstractPage
{
    /**
    ...
    public function __viewIndex($resource_type)
    {
        ...
        $friendly_resource = (\ResourceManager::RESOURCE_TYPE_EVENT === $resource_type) ? __('Event') : __('DataSource');
        ...
        if (!is_array($resources) || empty($resources)) {
            ...
            foreach ($resources as $r) {
                ...
                // Locked resources
                if (
                    isset($r['can_parse']) && true !== $r['can_parse'] ||
                    (\ResourceManager::RESOURCE_TYPE_DS === $resource_type && 'Dynamic_xml' === $r['source']['name'])
                ) {
                    $action = 'info';
                ...
                foreach ($pages as $p) {
                    ...
                    )->generate().(count($pages) > $i ? (0 == ($i % 10) ? '<br />' : ', ') : '');
                }
        ...
        if (isset($_POST['action']) && is_array($_POST['action'])) {
            ...
            if (is_array($checked) && !empty($checked)) {
                if ('delete' == $_POST['with-selected']) {
                    ...
                    if ('detach' == substr($_POST['with-selected'], 0, 6)) {
