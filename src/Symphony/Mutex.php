...
class Mutex
{
    ...
    {
        $lockFile = self::generateLockFileName($id, $path);

        // If this thread already has acquired the lock, return true.
        if (true == isset(self::$lockFiles[$lockFile])) {
     ...
     * @return bool
     */
    public static function release($id, $path = '.'): bool
    {
        $lockFile = self::generateLockFileName($id, $path);
     ...
     * @return bool
     */
    public static function refresh($id, $ttl = 5, $path = '.'): bool
    {
        return touch(self::generateLockFileName($id, $path), time() + $ttl, time());
    }
     ...
     * @return bool
     */
    public static function lockExists($id, $path): bool
    {
        $lockFile = self::generateLockFileName($id, $path);

        return true == file_exists($lockFile);
    }
     ...
     * @return string
     */
    private static function generateLockFileName($id, $path = null): string
    {
        // This function is called from all others, so it is a good point to initialize Mutex handling.
        if (false == is_array(self::$lockFiles)) {
            self::$lockFiles = [];
            register_shutdown_function([__CLASS__, 'shutdownCleanup']);
        }
    ...
    public static function shutdownCleanup(): void
    {
        ...
        if (true == is_array(self::$lockFiles)) {
            foreach (self::$lockFiles as $lockFile => $meta) {
                if (($now - $meta['time'] > $meta['ttl']) && true == file_exists($lockFile)) {
                    unlink($lockFile);
                }
            }
        }
