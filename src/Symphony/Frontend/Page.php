...
class Page extends Symphony\AbstractXSLTPage
{
    ...
    /**
     * Constructor function sets the `$isLoggedIn` variable.
     */
    public function __construct()
    {
        parent::__construct();

        $this->isLoggedIn = \Frontend::instance()->isLoggedIn();
    }
    ...
    /**
     * Accessor function for the resolved page's data (`$this->pageData`)
    ...
    public function generate($page = null)
    {
        ...
        if (null !== $devkit) {
            $devkit->prepare($this, $this->pageData, $this->_xml, $this->_param, $output);
    ...
    private function __buildPage()
    {
        ...
        if (is_array($_COOKIE[__SYM_COOKIE_PREFIX__]) && !empty($_COOKIE[__SYM_COOKIE_PREFIX__])) {
            foreach ($_COOKIE[__SYM_COOKIE_PREFIX__] as $key => $val) {
                if ('xsrf-token' === $key && is_array($val)) {
                    $val = key($val);
                }
        ...
        foreach ($this->_param as $key => $value) {
            ...
            if (is_array($value) && !(1 == count($value) && empty($value[0]))) {
    ...
    public function resolvePage($page = null)
    {
        ...
        // Not the index page (or at least not on first impression)
            ...
            } while (null !== ($handle = array_pop($pathArr)));
        ...
        if (!$this->isLoggedIn && in_array('admin', $row['type'])) {
    ...
    public function processDatasources($datasources, \XMLElement &$wrapper, array $params = array())
    {
        ...
        foreach ($dsOrder as $handle) {
            ...
            } catch (Exceptions\FrontendPageNotFoundException $e) {
                // Work around. This ensures the 404 page is displayed and
                // is not picked up by the default catch() statement below
                PageNotFoundExceptionHandler::render($e);
            }
