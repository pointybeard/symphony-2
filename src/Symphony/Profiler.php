 ...
 */
class Profiler implements Interfaces\SingletonInterface
{
    ...
    {
        self::$_starttime = precision_timer();
    }
    ...
    {
        self::$_seed = (null === $time) ? precision_timer() : $time;
    }
    ...
    {
        if (PROFILE_RUNNING_TOTAL == $type) {
            self::$_samples[] = array($msg, precision_timer('stop', self::$_starttime), precision_timer(), $group, $queries, memory_get_usage());
        } else {
            if (null !== self::$_seed) {
                $start = self::$_seed;
                self::$_seed = null;
            ...
            $prev = self::retrieveLast();
            self::$_samples[] = array($msg, precision_timer('stop', ($start ? $start : $prev[2])), precision_timer(), $group, $queries, memory_get_usage());
        }
    }
    ...
    {
        return null !== $index ? self::$_samples[$index] : self::$_samples;
    }
    ...
    {
        foreach (self::$_samples as $record) {
            if ($record[0] == $msg) {
                return $record;
            }
        }
    ...
    {
        return precision_timer('stop', self::$_starttime);
    }
