...
class Entry
{
     ...
     * @var int
     */
    public const __ENTRY_FIELD_ERROR__ = 100;
    ...
    public function assignEntryId()
    {
        ...
        $fields['author_id'] = null === $this->get('author_id') ? 1 : (int) $this->get('author_id'); // Author_id cannot be null
        $fields['modification_author_id'] = null === $this->get('modification_author_id') ? $fields['author_id'] : (int) $this->get('modification_author_id');
    ...
    public function setDataFromPost($data, &$errors = null, $simulate = false, $ignore_missing_fields = false)
    {
        ...
        if (self::__ENTRY_OK__ !== $status && null !== $entry_id) {
    ...
    {
        if (null === $this->get('section_id')) {
            return null;
        }

        if (null === $associated_sections) {
            $section = \SectionManager::fetch($this->get('section_id'));
