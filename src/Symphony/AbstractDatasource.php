 ...
 */
abstract class AbstractDatasource implements Interfaces\DatasourceInterface
{
    /**
    ...
    /**
     * Constructor for the datasource sets the parent, if `$processParams` is set,
     * the `$env` variable will be run through `self::processParameters`.
     *
    ...
    public function __construct(array $env = null, bool $processParams = true)
    {
        ...
        if (3 == count($arguments) && is_bool($arguments[1]) && is_bool($arguments[2])) {
            $env = $arguments[0];
            $processParams = $arguments[1];
        }
     ...
     *
     * @return int
     *             self::FILTER_OR or self::FILTER_AND
     */
    public static function determineFilterType($value)
    {
        ...
        return 1 === preg_match($pattern, $value) ? self::FILTER_AND : self::FILTER_OR;
    }
     ...
     *                            Valid values are self::FILTER_OR or self::FILTER_AND
     * @param string $value
     *                            The filter's value
     *
     * @return array
    ...
    {
        $pattern = self::FILTER_AND === $filter_type ? '\+' : '(?<!\\\\),';
    ...
    /**
     * If there is no results to return this function calls `self::__noRecordsFound`
    ...
    /**
     * If the datasource has been negated this function calls `self::__negateResult`
     * which appends an \XMLElement to the current root element.
     *
     * @param \XMLElement $xml
     *                         The root element \XMLElement for this datasource. By default, this will
    ...
    public function processParameters(array $env = null)
    {
        ...
        if ((isset($this->_env) && is_array($this->_env)) && isset($this->dsParamFILTERS) && is_array($this->dsParamFILTERS) && !empty($this->dsParamFILTERS)) {
            foreach ($this->dsParamFILTERS as $key => $value) {
                ...
                if (0 === strlen(trim($new_value)) || !preg_match('/[^\s|+|,]+/u', $new_value)) {
        ...
        if (isset($this->dsParamREDIRECTONEMPTY) && 'yes' === $this->dsParamREDIRECTONEMPTY && $this->_force_empty_result) {
        ...
        if (preg_match_all('@{([^}]+)}@i', $value, $matches, PREG_SET_ORDER)) {
            foreach ($matches as $match) {
                ...
                foreach ($bits as $param) {
                    ...
                    $replacement = self::findParameterInEnv($param, $env);
