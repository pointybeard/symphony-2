...
/**
 * The Administration\AbstractPage class represents a Symphony backend page.
 * It extends the HTMLPage class and, unlike the Frontend, is generated
 * using a number of Symphony\XmlElement objects. Instances of this class override
 ...
 */
abstract class AbstractPage extends Symphony\HtmlPage
{
    /**
     ...
     * the basic HTML, Head and Body `Symphony\XmlElement`'s. This function
     * also sets the `Symphony\XmlElement` element style to be HTML, instead of XML.
     */
    ...
    {
        $this->setBodyClass('form' == $type || 'single' == $type ? 'single' : 'index');
    }
    ...
    public function setBodyClass($class)
    {
        ...
        if (!isset($this->_context['page']) || 'index' !== $this->_context['page'] || 'index' !== $class) {
    ...
    {
        if (null === $message && Alert::ERROR == $type) {
        ...
        if (0 == strlen(trim($message))) {
            throw new Exceptions\SymphonyException(__('A message must be supplied unless the alert is of type Alert::ERROR'));
        }
    ...
    public function appendSubheading($value, $actions = null)
    {
        ...
        $this->Breadcrumbs->appendChild(new Symphony\XmlElement('h2', $value, array('role' => 'heading', 'id' => 'symphony-subheading')));
    }

    /**
     * This function allows a user to insert an Action button to the page.
     * It accepts an `Symphony\XmlElement` (which should be of the `Anchor` type),
    ...
    public function insertAction(Symphony\XmlElement $action, $append = true)
    {
        ...
        if (empty($actions)) {
            $ul = new Symphony\XmlElement('ul', null, array('class' => 'actions'));
        ...
        $li = new Symphony\XmlElement('li', $action);
     ...
     *
     * @param array $values
     *                      An array of `Symphony\XmlElement`'s or strings that compose the path. If breadcrumbs
    ...
    public function insertBreadcrumbs(array $values)
    {
        ...
        if ($this->Breadcrumbs instanceof Symphony\XmlElement && 1 === count($this->Breadcrumbs->getChildrenByName('nav'))) {
            $nav = $this->Breadcrumbs->getChildrenByName('nav');
            $nav = $nav[0];

            $p = $nav->getChild(0);
        } else {
            $p = new Symphony\XmlElement('p');
            $nav = new Symphony\XmlElement('nav');
            $nav->appendChild($p);
        ...
        foreach ($values as $v) {
            $p->appendChild($v);
            $p->appendChild(new Symphony\XmlElement('span', '&#8250;', array('class' => 'sep')));
        }
    }
     ...
     */
    public function insertDrawer(Symphony\XmlElement $drawer, $position = 'horizontal', $button = 'append')
    {
    ...
    public function build(array $context = array())
    {
        ...
        if (!$this->canAccessPage()) {
            \Administration::instance()->throwCustomError(
                ...
                __('Access Denied'),
                Symphony\AbstractPage::HTTP_STATUS_UNAUTHORIZED
            );
        }

        $this->Html->setDTD('<!DOCTYPE html>');
        $this->Html->setAttribute('lang', Symphony\Lang::get());
        $this->addElementToHead(new Symphony\XmlElement('meta', null, array('charset' => 'UTF-8')), 0);
        $this->addElementToHead(new Symphony\XmlElement('meta', null, array('http-equiv' => 'X-UA-Compatible', 'content' => 'IE=edge,chrome=1')), 1);
        $this->addElementToHead(new Symphony\XmlElement('meta', null, array('name' => 'viewport', 'content' => 'width=device-width, initial-scale=1')), 2);
        ...
        $this->addElementToHead(
            new Symphony\XmlElement('script', json_encode($environment, $envJsonOptions), array(
        ...
        $this->Wrapper = new Symphony\XmlElement('div', null, array('id' => 'wrapper'));
        $this->Header = new Symphony\XmlElement('header', null, array('id' => 'header'));
        $this->Context = new Symphony\XmlElement('div', null, array('id' => 'context'));
        $this->Breadcrumbs = new Symphony\XmlElement('div', null, array('id' => 'breadcrumbs'));
        $this->Contents = new Symphony\XmlElement('div', null, array('id' => 'contents', 'role' => 'main'));
        $this->Form = Symphony\Widget::Form(\Administration::instance()->getCurrentPageURL(), 'post', null, null, array('role' => 'form'));
        ...
        $h1 = new Symphony\XmlElement('h1');
        $h1->appendChild(Symphony\Widget::Anchor(\Symphony::Configuration()->get('sitename', 'general'), rtrim(URL, '/').'/'));
     ...
     * @uses CanAccessPage
     *
     * @see http://github.com/symphonycms/symphony-2/blob/master/symphony/assets/xml/navigation.xml
     *
     * @return bool
    ...
    public function canAccessPage()
    {
        ...
        foreach ($nav as $item) {
            if (
                ...
                Symphony\General::in_array_multi($page, $item['children'])
                // If the page namespace matches one of the children (this will usually drop query
                // string parameters such as /edit/1/)
                || Symphony\General::in_array_multi(\Symphony::getPageNamespace().'/', $item['children'])
            ) {
                ...
                if (isset($item['limit']) && 'primary' !== $page_limit) {
                    if ('author' === $page_limit && 'developer' === $item['limit']) {
        ...
        if ($hasAccess) {
            ...
            \Symphony::ExtensionManager()->notifyMembers('CanAccessPage', '/backend/', array(
                ...
                'page_url' => $page,
                'section' => array(
                    'id' => !$section_handle ? 0 : Symphony\Managers\SectionManager::fetchIDFromHandle($section_handle),
    ...
    public function doesAuthorHaveAccess($item_limit = null)
    {
        ...
        if (!isset($item_limit) || 'author' === $item_limit) {
            $can_access = true;
        } elseif ('developer' === $item_limit && \Symphony::Author()->isDeveloper()) {
            $can_access = true;
        } elseif ('manager' === $item_limit && (\Symphony::Author()->isManager() || \Symphony::Author()->isDeveloper())) {
            $can_access = true;
        } elseif ('primary' === $item_limit && \Symphony::Author()->isPrimaryAccount()) {
            $can_access = true;
        }
     ...
     * parent `generate()` is called which will convert the `Symphony\XmlElement`'s
    ...
    private function appendBodyClass(array $context = array())
    {
        ...
        $classes = array_merge(explode(' ', trim($body_class)), explode(' ', trim($this->bodyClass)));
    ...
    public function __switchboard($type = 'view')
    {
        ...
        $function = ('action' == $type ? '__action' : '__view').ucfirst($context);
    ...
    public function appendAlert()
    {
        ...
        foreach ($this->Alert as $alert) {
            $this->Header->prependChild($alert->toXml());
        }
    }
    ...
    public function sortAlerts($a, $b)
    {
        ...
        if (
            (Alert::ERROR === $a->{'type'} && $a->{'type'} !== $b->{'type'})
            || (Alert::SUCCESS === $a->{'type'} && Alert::NOTICE === $b->{'type'})
        ) {
            return -1;
        }
    ...
    public function appendNavigation()
    {
        ...
        $navElement = new Symphony\XmlElement('nav', null, array('id' => 'nav', 'role' => 'navigation'));
        $contentNav = new Symphony\XmlElement('ul', null, array('class' => 'content', 'role' => 'menubar'));
        $structureNav = new Symphony\XmlElement('ul', null, array('class' => 'structure', 'role' => 'menubar'));

        foreach ($nav as $n) {
            if (isset($n['visible']) && 'no' === $n['visible']) {
                continue;
            }
            ...
            if ($this->doesAuthorHaveAccess($item_limit)) {
                $xGroup = new Symphony\XmlElement('li', Symphony\General::sanitize($n['name']), array('role' => 'presentation'));
                ...
                $hasChildren = false;
                $xChildren = new Symphony\XmlElement('ul', null, array('role' => 'menu'));

                if (is_array($n['children']) && !empty($n['children'])) {
                    foreach ($n['children'] as $c) {
                        // adapt for Yes and yes
                        if ('yes' !== strtolower($c['visible'])) {
                            continue;
                        }
                        ...
                        if ($this->doesAuthorHaveAccess($child_item_limit)) {
                            $xChild = new Symphony\XmlElement('li');
                            $xChild->setAttribute('role', 'menuitem');
                            $linkChild = Symphony\Widget::Anchor(Symphony\General::sanitize($c['name']), SYMPHONY_URL.$c['link']);
                            if (isset($c['target'])) {
            ...
            if (count($children) > 0) {
                foreach ($children as $child) {
                    $item = array(
                        ...
                        'visible' => ('no' == (string) $child->attributes()->visible ? 'no' : 'yes'),
                    );
    ...
    private function buildSectionNavigation(&$nav)
    {
        ...
        $sections = Symphony\Managers\SectionManager::fetch(null, 'asc', 'sortorder');
        ...
        if (is_array($sections) && !empty($sections)) {
            foreach ($sections as $s) {
                ...
                if (false === $group_index) {
                    $group_index = Symphony\General::array_find_available_index($nav, 0);
                ...
                if ($hasAccess) {
                    $nav[$group_index]['children'][] = array(
                        ...
                        'visible' => ('no' == $s->get('hidden') ? 'yes' : 'no'),
                    );
                }
            }
        }
                ...
                foreach ($extension_navigation as $item) {
                    ...
                    switch ($type) {
                        case \Extension::NAV_GROUP:
                            $index = Symphony\General::array_find_available_index($nav, $item['location']);
    ...
    {
        if (!isset($item['relative']) || true === $item['relative']) {
        ...
        $nav_item = array(
            'link' => $link,
            'name' => $item['name'],
            'visible' => (isset($item['visible']) && 'no' == $item['visible']) ? 'no' : 'yes',
            'limit' => isset($item['limit']) ? $item['limit'] : null,
     ...
     * @uses NavigationPostBuild
     *
     * @see https://github.com/symphonycms/symphony-2/blob/master/symphony/assets/xml/navigation.xml
     * @see https://github.com/symphonycms/symphony-2/blob/master/symphony/lib/toolkit/class.extension.php
     */
     ...
     * footer HTML, which is an Symphony\XmlElement of a `<ul>` element.
     * Since Symphony 2.3, it no longer uses the `AddElementToFooter` delegate.
     */
    public function appendUserLinks()
    {
        $ul = new Symphony\XmlElement('ul', null, array('id' => 'session'));
        ...
        $li = new Symphony\XmlElement('li');
        $li->appendChild(Symphony\Widget::Anchor(__('Log out'), SYMPHONY_URL.'/logout/', null, null, null, array('accesskey' => 'l')));
        $ul->appendChild($li);
    ...
    public function addTimestampValidationPageAlert($errorMessage, $existingObject, $action)
    {
        ...
        $msg = $this->_errors['timestamp'].' '.__(
            ...
            array(
                $formatteAuthorName,
                Symphony\Widget::Time($existingObject->get('modification_date'))->generate(),
            )
        );

        $currentUrl = \Administration::instance()->getCurrentPageURL();
        $overwritelink = Symphony\Widget::Anchor(
            __('Replace changes?'),
            $currentUrl,
            __('Overwrite'),
            'js-tv-overwrite',
            null,
            array(
                'data-action' => Symphony\General::sanitize($action),
            )
        );
        $ignorelink = Symphony\Widget::Anchor(
            __('View changes.'),
            $currentUrl,
