 ...
 */
abstract class AbstractEmailGateway
{
     ...
     *
     * @throws Exceptions\EmailValidationException
     */
    ...
    public function setSenderEmailAddress($email)
    {
        if (!$this->validateHeaderFieldValue($email)) {
            throw new Exceptions\EmailValidationException(__('Sender Email Address can not contain carriage return or newlines.'));
        }
    ...
    public function setSenderName($name)
    {
        if (!$this->validateHeaderFieldValue($name)) {
            throw new Exceptions\EmailValidationException(__('Sender Name can not contain carriage return or newlines.'));
        }
    ...
    public function setRecipients($email)
    {
        ...
        foreach ($email as $e) {
            if (!$this->validateHeaderFieldValue($e)) {
                throw new Exceptions\EmailValidationException(__('Recipient address can not contain carriage return or newlines.'));
            }
        }
     ...
     * @since Symphony 2.7
     *
     * @param bool $validate_attachment_errors
     */
    public function setValidateAttachmentErrors($validate_attachment_errors)
    {
        if (!is_bool($validate_attachment_errors)) {
            throw new Exceptions\EmailGatewayException(__('%s accepts boolean values only.', array('<code>setValidateAttachmentErrors</code>')));
        } else {
    ...
    public function setTextEncoding($encoding = null)
    {
        if ('quoted-printable' == $encoding) {
            ...
            throw new Exceptions\EmailGatewayException(__('%1$s is not a supported encoding type. Please use %2$s or %3$s. You can also use %4$s for no encoding.', array($encoding, '<code>quoted-printable</code>', '<code>base-64</code>', '<code>false</code>')));
        }
    }
    ...
    {
        if (preg_match('%[\r\n]%', $email)) {
            throw new Exceptions\EmailValidationException(__('Reply-To Email Address can not contain carriage return or newlines.'));
        }
    ...
    {
        if (preg_match('%[\r\n]%', $name)) {
            throw new Exceptions\EmailValidationException(__('Reply-To Name can not contain carriage return or newlines.'));
        }
    ...
    {
        if (is_array($body)) {
            throw new Exceptions\EmailGatewayException(__('%s accepts strings only; arrays are not allowed.', array('<code>appendHeaderField</code>')));
        }
    ...
    public function validate()
    {
        if (strlen(trim($this->_subject)) <= 0) {
            throw new Exceptions\EmailValidationException(__('Email subject cannot be empty.'));
        } elseif (strlen(trim($this->_sender_email_address)) <= 0) {
            throw new Exceptions\EmailValidationException(__('Sender email address cannot be empty.'));
        } elseif (!filter_var($this->_sender_email_address, FILTER_VALIDATE_EMAIL)) {
            throw new Exceptions\EmailValidationException(__('Sender email address must be a valid email address.'));
        } else {
            foreach ($this->_recipients as $address) {
                if (strlen(trim($address)) <= 0) {
                    throw new Exceptions\EmailValidationException(__('Recipient email address cannot be empty.'));
                } elseif (!filter_var($address, FILTER_VALIDATE_EMAIL)) {
                    throw new Exceptions\EmailValidationException(__('The email address ‘%s’ is invalid.', array($address)));
                }
            }
        }
    ...
    protected function prepareMessageBody()
    {
        ...
        if ($attachments) {
            ...
            throw new Exceptions\EmailGatewayException(__('No attachments or body text was set. Can not send empty email.'));
        }
    }
        ...
        foreach ($this->_attachments as $key => $file) {
            ...
            if (false !== $file_content && !empty($file_content)) {
                ...
                if ($this->_validate_attachment_errors) {
                    ...
                    throw new Exceptions\EmailGatewayException(__('The content of the file `%s` could not be loaded.', array($filename)));
                }
            }
    ...
    {
        if ('quoted-printable' == $this->_text_encoding) {
            return EmailHelper::qpContentTransferEncode($this->_text_plain)."\r\n";
        } elseif ('base64' == $this->_text_encoding) {
    ...
    public function contentInfoArray($type = null, $file = null, $filename = null, $charset = null)
    {
        ...
        $filename = null !== $filename ? $filename : basename($file);
    ...
    public function __set($name, $value)
    {
        if (method_exists(get_class($this), 'set'.$this->__toCamel($name, true))) {
            ...
            throw new Exceptions\EmailGatewayException(__('The %1$s gateway does not support the use of %2$s', array(get_class($this), $name)));
        }
    }
