...
class contentBlueprintsDatasources extends ResourcesPage
{
    ...
    public function __form()
    {
        ...
        if (isset($_POST['fields'])) {
            ...
            $fields['order'] = ('rand' == $order) ? 'random' : $order;
        ...
        $options = array(
            array('label' => __('System'), 'data-label' => 'system', 'options' => array(
                    array('authors', ('authors' == $fields['source']), __('Authors'), null, null, array('data-context' => 'authors')),
                    array('navigation', ('navigation' == $fields['source']), __('Navigation'), null, null, array('data-context' => 'navigation')),
            )),
            array('label' => __('Custom XML'), 'data-label' => 'custom-xml', 'options' => array(
                    array('static_xml', ('static_xml' == $fields['source']), __('Static XML'), null, null, array('data-context' => 'static-xml')),
            )),
        );
        ...
        $options = array(
            array('label' => __('Authors'), 'data-label' => 'authors', 'options' => array(
                    array('id', ('authors' == $fields['source'] && 'id' == $fields['sort']), __('Author ID')),
                    array('username', ('authors' == $fields['source'] && 'username' == $fields['sort']), __('Username')),
                    array('first-name', ('authors' == $fields['source'] && 'first-name' == $fields['sort']), __('First Name')),
                    array('last-name', ('authors' == $fields['source'] && 'last-name' == $fields['sort']), __('Last Name')),
                    array('email', ('authors' == $fields['source'] && 'email' == $fields['sort']), __('Email')),
                    array('status', ('authors' == $fields['source'] && 'status' == $fields['sort']), __('Status')),
                ),
            ),

            array('label' => __('Navigation'), 'data-label' => 'navigation', 'options' => array(
                    array('id', ('navigation' == $fields['source'] && 'id' == $fields['sort']), __('Page ID')),
                    array('handle', ('navigation' == $fields['source'] && 'handle' == $fields['sort']), __('Handle')),
                    array('sortorder', ('navigation' == $fields['source'] && 'sortorder' == $fields['sort']), __('Sort Order')),
                ),
            ),
        );
        ...
        foreach ($field_groups as $section_id => $section_data) {
            $optgroup = array('label' => General::sanitize($section_data['section']->get('name')), 'data-label' => 'section-'.$section_data['section']->get('id'), 'options' => array(
                array('system:id', ($fields['source'] == $section_id && 'system:id' == $fields['sort']), __('System ID')),
                array('system:creation-date', ($fields['source'] == $section_id && ('system:creation-date' == $fields['sort'] || 'system:date' == $fields['sort'])), __('System Creation Date')),
                array('system:modification-date', ($fields['source'] == $section_id && 'system:modification-date' == $fields['sort']), __('System Modification Date')),
            ));
        ...
        $options = array(
            array('label' => __('Authors'), 'data-label' => 'authors', 'options' => array(
                    array('username', ('authors' == $fields['source'] && in_array('username', $fields['xml_elements'])), 'username'),
                    array('name', ('authors' == $fields['source'] && in_array('name', $fields['xml_elements'])), 'name'),
                    array('email', ('authors' == $fields['source'] && in_array('email', $fields['xml_elements'])), 'email'),
                    array('author-token', ('authors' == $fields['source'] && in_array('author-token', $fields['xml_elements'])), 'author-token'),
                    array('default-area', ('authors' == $fields['source'] && in_array('default-area', $fields['xml_elements'])), 'default-area'),
            )),
        );
        ...
        foreach (array('id', 'username', 'name', 'email', 'user_type') as $p) {
            $options[0]['options'][] = array(
                $p,
                ('authors' == $fields['source'] && in_array($p, $fields['param'])),
                $prefix.$p,
                null,
                null,
                array(
    ...
    public function __viewInfo()
    {
        ...
        $this->appendSubheading((('info' == $this->_context[0]) ? $about['name'] : __('Untitled')));
        $this->insertBreadcrumbs(array(
            Widget::Anchor(__('Data Sources'), SYMPHONY_URL.'/blueprints/datasources/'),
        ));
    ...
    public function __formAction()
    {
        ...
        } elseif (false !== strpos($fields['name'], '\\')) {
        ...
        if ('static_xml' == $fields['source']) {
            if ('' == trim($fields['static_xml'])) {
                $this->_errors['static_xml'] = __('This is a required field');
            } else {
                $xml_errors = null;

                General::validateXML($fields['static_xml'], $xml_errors, false);
            ...
            if (0 == strlen(trim($fields['max_records'])) || (is_numeric($fields['max_records']) && $fields['max_records'] < 1)) {
                if ('yes' === $fields['paginate_results']) {
            ...
            if (0 == strlen(trim($fields['page_number'])) || (is_numeric($fields['page_number']) && $fields['page_number'] < 1)) {
                if ('yes' === $fields['paginate_results']) {
        ...
        if ('new' == $this->_context[0] && is_file($file)) {
            $isDuplicate = true;
        } elseif ('edit' == $this->_context[0]) {
        ...
        if (empty($this->_errors)) {
            ...
            if ($providerClass) {
                ...
                switch ($source) {
                    ...
                    case 'static_xml':
                        ...
                        if (true == preg_match('/^<\?xml/i', $fields['static_xml'])) {
            ...
            // Write successful
                ...
                ResourceManager::setPages(ResourceManager::RESOURCE_TYPE_DS, null === $existing_handle ? $classname : $existing_handle, $connections);
                ...
                redirect(SYMPHONY_URL.'/blueprints/datasources/edit/'.$classname.'/'.('new' == $this->_context[0] ? 'created' : 'saved').'/');
            }
        }
    }
